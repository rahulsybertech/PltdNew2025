...
public class SupplierAdapter extends RecyclerView.Adapter<SupplierAdapter.MyViewHolder>{
    ...
    public SupplierAdapter(Context mContext, List<FilterListResult> detailList) {
    ...
    @Override
    public void onBindViewHolder(SupplierAdapter.MyViewHolder holder, final int position) {

        final FilterListResult datum = filterListDetails.get(position);
        holder.filter.setText(datum.getFilterName().equals("")?"Without Supllier":datum.getFilterName());
...
//        });
        ...
        List<com.syber.ssspltd.response.PendingOrdBranchRespo.SupplierRespo.FilterListResult> isSelected = filterListDetails.stream().filter(p -> p.isSelected()).collect(Collectors.toList());
        try {
            PendingOrderActivity.countbrand.setText(isSelected.size()+"");
        }catch (Exception e){

        }
        List<com.syber.ssspltd.response.PendingOrdBranchRespo.SupplierRespo.FilterListResult> isSelected2 = filterListDetails.stream().filter(p -> p.isSelected()).collect(Collectors.toList());
        try {
            StockInOfficeActivity.countbrand.setText(isSelected2.size()+"");
        }catch (Exception e){
