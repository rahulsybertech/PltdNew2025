...
import com.syber.ssspltd.response.CourierNameRespons.CourierNoRespo.FilterListResult;

import java.util.List;
import java.util.stream.Collectors;

public class CourierNoAdp extends RecyclerView.Adapter<CourierNoAdp.MyViewHolder>{
    private Context mContext;
    private List<FilterListResult> filterListDetails;

    public CourierNoAdp(Context mContext, List<FilterListResult> detailList) {
        this.mContext = mContext;
        this.filterListDetails = detailList;
    }

    @Override
    public CourierNoAdp.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.filter_recyler, parent, false);
        return new CourierNoAdp.MyViewHolder(v);
    }

    @RequiresApi(api = Build.VERSION_CODES.N)
    @Override
    public void onBindViewHolder(CourierNoAdp.MyViewHolder holder, final int position) {

        final FilterListResult datum = filterListDetails.get(position);
        holder.filter.setText(datum.getFilterName().equals("")?"Without Number":datum.getFilterName());
...
//        });
        ...
        List<com.syber.ssspltd.response.CourierNameRespons.CourierNoRespo.FilterListResult> isSelected2 = filterListDetails.stream().filter(p -> p.isSelected()).collect(Collectors.toList());
        try {
            CourierReportActivity.count_CNo.setText(isSelected2.size()+"");
        }catch (Exception e){
