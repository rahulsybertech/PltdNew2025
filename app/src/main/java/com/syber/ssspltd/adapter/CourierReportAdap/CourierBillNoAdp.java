package com.syber.ssspltd.adapter.CourierReportAdap;
...
import com.syber.ssspltd.response.CourierNameRespons.CourierBillNoRespo.FilterListResult;

import java.util.List;
import java.util.stream.Collectors;

public class CourierBillNoAdp  extends RecyclerView.Adapter<CourierBillNoAdp.MyViewHolder>{
    private Context mContext;
    private List<FilterListResult> filterListDetails;

    public CourierBillNoAdp(Context mContext, List<FilterListResult> detailList) {
        this.mContext = mContext;
        this.filterListDetails = detailList;
    }

    @Override
    public CourierBillNoAdp.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.filter_recyler, parent, false);
        return new CourierBillNoAdp.MyViewHolder(v);
    }

    @RequiresApi(api = Build.VERSION_CODES.N)
    @Override
    public void onBindViewHolder(CourierBillNoAdp.MyViewHolder holder, final int position) {

        final FilterListResult datum = filterListDetails.get(position);
        holder.filter.setText(datum.getFilterName().equals("")?"Without BillNo":datum.getFilterName());
...
//        });
        ...
        List<com.syber.ssspltd.response.CourierNameRespons.CourierBillNoRespo.FilterListResult> isSelected3 = filterListDetails.stream().filter(p -> p.isSelected()).collect(Collectors.toList());
        try {
            CourierReportActivity.count_CBill_No.setText(isSelected3.size()+"");
        }catch (Exception e){
