...
import com.syber.ssspltd.response.CourierNameRespons.FilterListResult;

import java.util.List;
import java.util.stream.Collectors;

public class CourierNameAdp extends RecyclerView.Adapter<CourierNameAdp.MyViewHolder>{
    private Context mContext;
    private List<FilterListResult> filterListDetails;

    public CourierNameAdp(Context mContext, List<FilterListResult> detailList) {
        this.mContext = mContext;
        this.filterListDetails = detailList;
    }

    @Override
    public CourierNameAdp.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.filter_recyler, parent, false);
        return new CourierNameAdp.MyViewHolder(v);
    }

    @RequiresApi(api = Build.VERSION_CODES.N)
    @Override
    public void onBindViewHolder(CourierNameAdp.MyViewHolder holder, final int position) {

        final FilterListResult datum = filterListDetails.get(position);
        holder.filter.setText(datum.getFilterName().equals("")?"Without Name":datum.getFilterName());
...
//        });
        ...
        List<com.syber.ssspltd.response.CourierNameRespons.FilterListResult> isSelected1 = filterListDetails.stream().filter(p -> p.isSelected()).collect(Collectors.toList());
        try {
            CourierReportActivity.count_Cname.setText(isSelected1.size()+"");
        }catch (Exception e){
